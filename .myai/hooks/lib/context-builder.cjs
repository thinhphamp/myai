...
const USAGE_CACHE_FILE = path.join(os.tmpdir(), 'myai-usage-cache.json');
 ...
 */
function resolveRulesPath(filename, configDirName = '.myai') {
 ...
 * @returns {string|null} Resolved path or null
 */
function resolveScriptPath(filename, configDirName = '.myai') {
 ...
 */
function resolveSkillsVenv(configDirName = '.myai') {
...
function buildContextSection(sessionId) {
  ...
  try {
    const contextPath = path.join(os.tmpdir(), `myai-context-${sessionId}.json`);
 ...
 * }}
 */
function buildReminderContext({ sessionId, config, staticEnv, configDirName = '.myai', baseDir } = {}) {
