...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.myai/config.json` (or `~/.myai/config.json`) for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use WebSearch.
   - **Gemini Model**: Read from `.myai/config.json`: `gemini.model` (default: `gemini-3-flash-preview`)
