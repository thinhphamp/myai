/**
 * Shared utilities for myai hooks
 *
...
const LOCAL_CONFIG_PATH = '.myai/config.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.myai', 'config.json');
...
function getSessionTempPath(sessionId) {
  return path.join(os.tmpdir(), `myai-session-${sessionId}.json`);
}
 ...
 * @param {Object} config - myai config
 ...
 *   2. Global config (~/.myai/config.json) - user preferences
 *   3. Local config (./.myai/config.json) - project-specific overrides
 *
...
function resolveNamingPattern(planConfig, gitBranch) {
  ...
  if (!validation.valid) {
    ...
    if (process.env.MYAI_DEBUG) {
      console.error(`[myai-config] Warning: ${validation.error}`);
    }
  }
