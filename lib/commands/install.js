...
export async function install(options) {
  const { claude, opencode, gemini, codex, all, global: isGlobal } = options;
  ...
  const runtimes = [];
  if (all) {
    runtimes.push('claude', 'opencode', 'gemini', 'codex');
  } else {
  ...
  if (runtimes.length === 0) {
    ...
    logger.info('Usage: myai install [--claude] [--opencode] [--gemini] [--codex] [--all] [--global]');
    return;
  }
