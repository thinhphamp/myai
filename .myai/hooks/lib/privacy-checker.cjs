...
/**
 * Load config.json config to check if privacy block is disabled
 * @param {string} [configDir] - Directory containing config.json (defaults to .myai in cwd)
...
function isPrivacyBlockDisabled(configDir) {
  try {
    const configPath = configDir
      ...
      : path.join(process.cwd(), '.myai', 'config.json');
 ...
 * @param {string} [params.options.configDir] - Directory for config.json config
