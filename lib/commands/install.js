...
function installClaude(targetDir, pkgRoot, isGlobal) {
  ...
  // Set statusLine.command if not already configured
  const statuslineScript = path.join(targetDir, 'statusline.sh');
  if (fs.existsSync(statuslineScript)) {
    fs.chmodSync(statuslineScript, 0o755);
    const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
    if (!settings.statusLine?.command) {
      settings.statusLine = { command: statuslineScript };
      fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2) + '\n');
      logger.success(`Configured statusLine → ${statuslineScript}`);
    } else {
      logger.dim('statusLine already configured — skipped');
    }
  }
  ...
  if (!isGlobal) {
    ...
    const geminiMdSrc = path.join(pkgRoot, 'runtime-templates', 'gemini', 'GEMINI.md');
    const geminiMdDest = path.join(process.cwd(), 'GEMINI.md');
    if (!fs.existsSync(geminiMdDest)) {
      fs.copyFileSync(geminiMdSrc, geminiMdDest);
